const QUESTION_BANK = [
  ...
  { 
    subj: 1, 
    q: "하향식 접근 방식의 4단계 구성요소가 아닌 것은?", 
    choices: ["문제 탐색", "문제 정의", "문제 분석", "해결방안 탐색"], 
    ans: 2, 
    exp: "하향식 접근 방식의 4단계 구성요소로는 문제 탐색, 문제 정의, 해결방안 탐색, 타당성 평가가 있으며 순차적으로 진행됩니다." 
  },
  { 
    subj: 1, 
    q: "빅데이터 분석 방법론의 데이터 준비 단계 세부 태스크가 아닌 것은?",
    choices: ["필요 데이터 정의", "분석용 데이터 준비", "데이터 스토어 설계", "데이터 수집 및 정합성 점검"], 
    ans: 1, 
    exp: "데이터 준비 단계의 세부 태스크는 필요 데이터 정의, 데이터 스토어 설계, 데이터 수집 및 정합성 점검이 있으며, 분석용 데이터 준비 태스크는 데이터 분석 단계에 속합니다."
  },
  { 
    subj: 1, 
  ...
  { 
    subj: 1, 
    q: "정형 데이터 품질 기준 중 정확성의 세부 품질 기준 항목으로 적절하지 않은 것은?", 
    choices: ["선후 관계 정확성", "최신성", "참조 무결성", "업무 규칙 정확성"], 
    ans: 2, 
    exp: "정확성의 세부 품질 기준으로는 선후 관계 정확성, 계산/집계 정확성, 최신성, 업무 규칙 정확성이 있으며, 참조 무결성은 '일관성'의 세부 품질 기준이다."
  },
  { 
    subj: 1, 
  ...
  { 
    subj: 1, 
    q: "분석작업계획 수립 절차로 적절하지 않은 것은?", 
    choices: ["프로젝트 소요비용 배분", "프로젝트 작업분할구조 수립", "프로젝트 정의서 작성", "프로젝트 업무 분장 계획 및 배분"], 
    ans: 2, 
    exp: "분석작업계획 수립 절차로는 프로젝트 소요비용 배분, 프로젝트 작업분할 구조 수립, 프로젝트 업무 분장 계획 및 배분이 있습니다."
  },
  { 
    subj: 1, 
  ...
  { 
    ...
    choices: ["표본조사에서 전수조사로 변화", "사전처리에서 사후처리로 변화", "질(Quality)에서 양(Quantity)로의 강조로 변화", "상관관계에서 인과관계로 변화"], 
    ans: 3, 
  ...
  { 
    ...
    choices: ["데이터 범주화", "가명처리", "데이터 삭제", "데이터 마스킹"],
    ans: 2,
    exp: "데이터 삭제 기법으로, 데이터 공유나 개방 목적에 따라 데이터 셋에 구성된 값 중 필요 없는 값 또는 개인식별에 중요한 값을 삭제하는 방법입니다. 데이터 범주화는 데이터의 값을 범주의 값으로 변환하여 값을 숨기는 방법입니다. ex) 홍길동, 35세 → 홍씨, 30~40세"
  },
  {
    subj: 1, 
    q: "다음 중 빅데이터를 활용하는 과정에서 사생활 침해를 방지하기 위하여 데이터에 포함된 개인정보를 삭제하거나 알아볼 수 없는 형태로 변환하는 방법은?", 
    choices: ["가명화", "일반화", "정규화", "익명화"],
    ans: 3,
    exp: "익명화(Anonymization)는 사생활 침해를 방하기 위하여 데이터에 포함된 개인정보를 삭제하거나 알아볼 수 없는 형태로 변환하는 방법입니다."
  },
  { 
    subj: 1, 
  ...
  },
  {
    ...
    choices: ["VIF > 1이면 다중공선성 문제가 있다.", "VIF > 10이면 다중공선성 문제가 있다고 판단한다.", "VIF > 100이면 다중공선성 문제가 있다.", "VIF 값이 음수이면 다중공선성 문제가 있다."],
    ans: 1,
    exp: "VIF(분산팽창인수)는 특정 독립변수가 나머지 독립변수들에 의해 얼마나 설명되는지를 나타냅니다. 일반적으로 VIF > 10이면 심각한 다중공선성이 있다고 판단합니다. VIF=1은 공선성이 전혀 없음을 의미합니다. ☞ 암기 팁: VIF 경고 기준 = 10"
  },
  {
    subj: 3,
